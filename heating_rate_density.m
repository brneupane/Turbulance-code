function [q_MHD] = heating_rate_density(bx,by,bz,R,lat,delta_t)
    ...
    power_spectrum_tot_y = sum(cwt(B_fluct_perp(1,:), 1./(1.03*f_perp), 'morl').^2, 2).'; 
    power_spectrum_tot_z = sum(cwt(B_fluct_perp(1,:), 1./(1.03*f_perp), 'morl').^2, 2).'; 
